...
import qs from 'qs'
import {QueryResponseContextProps, QueryState, GridModel} from './models'
...
function calculateIsAllDataSelected<T>(data: Array<T> | undefined, selected: Array<string>): boolean {
  if (!data) {
    return false
  }
...
const getQueryData = (query: QueryState) => {
  ...
  if (query.filter) {
    data.append('filter', JSON.stringify(query.filter));
}
